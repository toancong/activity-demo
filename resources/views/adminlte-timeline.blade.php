...
<?php
    ...
    $labels = [
      ...
      'FriendRequest' => 'fa fa-user bg-aqua',
      'Comment'       => 'fa fa-comments bg-yellow',
      'PostImage'     => 'fa fa-camera bg-purple',
      'PostVideo'     => 'fa fa-video bg-maroon',
    ];
?>
<div class="wrapper">
      ...
      <div class="row">
        <div class="col-md-4">
          ...
          <ul class="timeline">
            ...
            <!-- timeline item -->
            <li>
            <i class="{{$labels[$activity->object_type]}}"></i>
              <div class="timeline-item">
                <span class="time"><i class="fa fa-clock"></i> {{\Carbon\Carbon::now()->diffForHumans($activity->published)}}</span>

                <h3 class="timeline-header">
                  {!!substitute($activity->summary, [
                    'actor.name'  => '<a href = "#">'.Arr::get($activity, 'actor.name').'</a>',
                  ])!!}
                </h3>

                @if($activity->content)
                <div class="timeline-body">
                    {!!$activity->content ? substitute($activity->content, [
                      'meta.object.content' => Arr::get($activity, 'meta.object.content'),
                    ]) : '' !!}
                </div>
                @endif

                @if($activity->object_type === 'PostImage')
                <div class="timeline-body">
                  @foreach (Arr::get($activity, 'meta.object.resource') as $url)
                  <img src="{{$url}}" class="margin">
                  @endforeach
                </div>
                @endif

                @if($activity->object_type === 'PostVideo')
                <div class="timeline-body">
                    <div class="embed-responsive embed-responsive-16by9">
                        <iframe class="embed-responsive-item" src="{{Arr::get($activity, 'meta.object.resource')}}"
